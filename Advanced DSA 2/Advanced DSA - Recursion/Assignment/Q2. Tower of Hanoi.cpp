...
void towerOfHanoiHelper(int N, vector<vector<int> > & res, int S, int H, int D ) {
    if (N == 0) return; 
    towerOfHanoiHelper(N-1, res, S, D, H);
    vector<int> temp;
    temp.push_back(N);
    temp.push_back(S);
    temp.push_back(D);
    res.push_back(temp);
    towerOfHanoiHelper(N-1, res, H, S, D);
}
vector<vector<int> > Solution::towerOfHanoi(int A) {

    vector<vector<int> > res;
    towerOfHanoiHelper(A, res, 1, 2, 3);
    return res;
}

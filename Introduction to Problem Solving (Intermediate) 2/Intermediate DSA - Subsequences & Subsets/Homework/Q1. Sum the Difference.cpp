...
int Solution::solve(vector<int> &A) {
    ...
    for(int i = 0; i < A.size(); i++) {
        minimum_sum = 2*minimum_sum + A[i];
        minimum_sum = minimum_sum % 1000000007;
        maximum_sum = 2*maximum_sum + A[A.size() - i - 1];
        maximum_sum = maximum_sum % 1000000007;
    }
    ans = (maximum_sum - minimum_sum + 1000000007) % 1000000007;
    return ans;
}
